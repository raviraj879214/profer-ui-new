  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/block-companies/${userid}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl w-full  p-5 relative flex flex-col">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        {/* Textarea */}
        <textarea
          placeholder="Type your block reason..."
...
<div className="mt-2 border-t border-gray-200 pt-3 flex flex-col gap-2">
  ...
  <button
    type="submit"
    className="w-30 bg-gray-900 text-white px-4 py-2 rounded-lg text-sm hover:bg-[#1E1E3E] self-end"
  >
