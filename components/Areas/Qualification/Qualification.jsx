...
const handleSubmit = async (e) => {
  ...
  try {
    if (editId) {
      ...
      if (res.ok) {
        setUpdateMessage({ text: "Qualification updated successfully!", type: "success" });
        setName("");
        setEditId(null);
        fetchQualifications();
      } else {
        setUpdateMessage({ text: data.message || "Error updating qualification", type: "error" });
      }
    } else {
      ...
      if (nameExists) {
        setUpdateMessage({ text: "Qualification name already exists", type: "error" });
   ...
   if (!name.trim()) {
    ...
    setNameError(""); // Clear if valid
      ...
      if (res.ok) {
        setUpdateMessage({ text: "Qualification added successfully!", type: "success" });
        setName("");
        fetchQualifications();                
      } else {
        setUpdateMessage({ text: data.message || "Error adding qualification", type: "error" });
      }
    }
  } catch (err) {
    console.error(err);
...
const handleDelete = async (ids) => {
  ...
  try {
    ...
    setDeleteRow(false); // Also correct setter here
  }
};
